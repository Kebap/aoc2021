...
class Map():
  ...
  def do_a_step(self):
    ...
    all_flashed_simultaneously = how_many_flashes_happened == len(self.points)
    ...
    return (how_many_flashes_happened, 
           all_flashed_simultaneously)
    

def solve(data, solve_part_b_as_well: bool):
  """
  ...
  first_flash_simultaneously = 0
  
  for i in range(100):
    flashes, all_flashed_simultaneously = m.do_a_step()
    total_flashes_after_100 += flashes
    logging.info(f"In this step #{i+1}, {flashes} squids flash, total of {total_flashes_after_100}")
    if solve_part_b_as_well and  all_flashed_simultaneously and first_flash_simultaneously == 0:
      first_flash_simultaneously = i

  if solve_part_b_as_well:
    while first_flash_simultaneously == 0:
      i += 1
      flashes, all_flashed_simultaneously = m.do_a_step()
      logging.info(f"In this step #{i+1}, {flashes} squids flash.")
      if all_flashed_simultaneously:
        first_flash_simultaneously = i + 1
    
  logging.info("Final configuration:")
  logging.info(str(m))
    
  return (total_flashes_after_100,
          first_flash_simultaneously)
