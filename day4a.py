...
def main():
  ...
  for number in called_numbers:
    print(f"Gezogen wurde die Nummer {number}.")
    for i, board in enumerate(boards):
      found = board.mark(int(number))
      if found:
        print(f"Brett {i} durfte ein Kreuz machen.")
      if board.test_bingo():
        print("BINGO!")
        bingo.append(board)
    if bingo:
      if len(bingo) > 1:
        print("Es gibt mehr als einen Gewinner!? Hier ist der Erste:")
      print(f"Das Brett mit den Zahlen {bingo[0].grid} erhält insgesamt {bingo[0].score(int(number))} Punkte. Glückwunsch!")
      break
